...
export async function POST(request: NextRequest) {
  try {
    ...
    const [createdHistory] = await db.insert(historyTable).values(newHistory).returning()
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deletedHistory = await db.delete(historyTable).where(eq(historyTable.id, id)).returning()
