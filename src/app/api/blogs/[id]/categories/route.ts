...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existingBlogCategory = await db
      ...
      .from(blogCategoriesTable)
      .where(
        and(eq(blogCategoriesTable.blogId, blogId), eq(blogCategoriesTable.categoryId, categoryId))
      )
      .limit(1)
    ...
    return NextResponse.json({ success: true, data: newBlogCategory }, { status: 201 })
  } catch (error) {
