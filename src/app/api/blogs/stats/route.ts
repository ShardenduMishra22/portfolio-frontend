...
export async function GET(request: NextRequest) {
  try {
    ...
    const authorStats = await db
      ...
      .leftJoin(usersTable, eq(blogTable.authorId, usersTable.id))
      .leftJoin(userProfilesTable, eq(usersTable.id, userProfilesTable.userId))
      .groupBy(blogTable.authorId, usersTable.email, userProfilesTable.firstName, userProfilesTable.lastName, userProfilesTable.avatar)
      .orderBy(desc(sql<number>`(
        SELECT COUNT(*) FROM ${blogViewsTable} 
        WHERE ${blogViewsTable.blogId} IN (
          SELECT id FROM ${blogTable} WHERE author_id = ${blogTable.authorId}
    ...
    const stats = {
      ...
      averageViewsPerPost: totalPosts[0]?.count ? Math.round((totalViews[0]?.count || 0) / totalPosts[0]?.count) : 0,
      averageLikesPerPost: totalPosts[0]?.count ? Math.round((totalLikes[0]?.count || 0) / totalPosts[0]?.count) : 0,
      averageCommentsPerPost: totalPosts[0]?.count ? Math.round((totalComments[0]?.count || 0) / totalPosts[0]?.count) : 0,
